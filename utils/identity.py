...
'''
refer to 
https://github.com/jayknoxqu/id-number-util/blob/master/identity.py
'''
...
class IdNumber(str):
    ...
    def generate_id(cls, sex=0):
        ...
        fml_60 = datetime.strptime(datetime.strftime(datetime.now() - timedelta(days=365*60),"%Y-%m-%d"),"%Y-%m-%d")
        male_55 = datetime.strptime(datetime.strftime(datetime.now() - timedelta(days=365*55),"%Y-%m-%d"),"%Y-%m-%d")
        adult = datetime.strptime(datetime.strftime(datetime.now() - timedelta(days=365*18),"%Y-%m-%d"),"%Y-%m-%d")
        female_45 = datetime.strptime(datetime.strftime(datetime.now() - timedelta(days=365*45),"%Y-%m-%d"),"%Y-%m-%d")
        start, end = datetime.strptime("1960-01-01", "%Y-%m-%d"), datetime.strptime("2000-12-30", "%Y-%m-%d")
        # if sex == None:
        #     birth_days = datetime.strftime(start + timedelta(random.randint(0, (end - start).days + 1)), "%Y%m%d")
        if sex % 2 == 1:
            birth_days = datetime.strftime(male_55 + timedelta(random.randint(0, (adult - male_55).days + 1)), "%Y%m%d")
        elif sex % 2 == 0:
            birth_days = datetime.strftime(female_45 + timedelta(random.randint(0, (adult - female_45).days + 1)), "%Y%m%d")
        else:
            birth_days = datetime.strftime(fml_60 + timedelta(random.randint(0, (adult - fml_60).days + 1)), "%Y%m%d")
...
if __name__ == '__main__':
    ...
    print(IdNumber.verify_id('610301196904051753'))  # 检验身份证是否正确:False